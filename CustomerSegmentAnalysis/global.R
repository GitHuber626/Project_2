...
# get_graph <- function(selected_independent, selected_dependent){
#   if (selected_dependent=='NumPhurchases'){
#     return(S_PL2)
#   } else if (selected_dependent=='MntProducts'){
#     return(S_PL)
#   }
  
# }

barplot1 <- S_PL%>%group_by(Education,MntProducts)%>%summarise(mean_prod=mean(Dollars))%>%ggplot(aes(x=MntProducts,Education))+geom_bar(aes(y=mean_prod,fill=Education),stat='identity',position='dodge')+labs(x='Product',y='Dollar',title='Education Impact on Purchase')


barplot2 <- S_PL%>%group_by(Marital_Status,MntProducts)%>%summarise(mean_prod=mean(Dollars))%>%
    ggplot(aes(x=MntProducts,Marital_Status))+geom_bar(aes(y=mean_prod,fill=Marital_Status),stat='identity',position
                                                       ='dodge')+labs(x='Product',y='Dollar',title='Marital Status Impact on Purchase')



barplot3 <- S_PL%>%group_by(Dt_Customer_Year,MntProducts)%>%summarise(mean_prod=mean(Dollars))%>%
    ggplot(aes(x=MntProducts,Dt_Customer_Year))+geom_bar(aes(y=mean_prod,fill=as.factor(Dt_Customer_Year)),stat='identity',position
                                                         ='dodge')+labs(x='Product',y='Dollar',title='Customer Enrollment Date Impact on Purchase')


barplot4 <- S_PL2%>%group_by(Education,NumPhurchases)%>%summarise(mean_freq=mean(Number))%>%
    ggplot(aes(x=NumPhurchases,Education))+geom_bar(aes(y=mean_freq,fill=Education),stat='identity',position
                                                    ='dodge')+labs(
                                                      x='Purchase Means',y=
                                                        'Avg Frequency',title=
...
barplot5 <- S_PL2%>%group_by(Marital_Status,NumPhurchases)%>%summarise(mean_freq=mean(Number))%>%
    ggplot(aes(x=NumPhurchases,Marital_Status))+geom_bar(aes(y=mean_freq,fill=Marital_Status),stat='identity',position
                                                         ='dodge')+labs(x='Purchase Means',y='Avg Frequency',title='Marital Status Impact on Purchase Frequency')



barplot6 <- S_PL2%>%group_by(Dt_Customer_Year,NumPhurchases)%>%summarise(mean_freq=mean(Number))%>%
    ggplot(aes(x=NumPhurchases,Dt_Customer_Year))+geom_bar(aes(y=mean_freq,fill=as.factor(Dt_Customer_Year)),stat='identity',position
                                                           ='dodge')+labs(x='Purchases Means',y='Avg Frequency',title='Customer Enrollment Date Impact on Purchase Frequency')


lineplot1 <- S_PL3%>%group_by(Dt_Customer_Year,NumPhurchases)%>%ggplot(aes(x=Dt_Customer_Year))+geom_line(aes(y=CPI_Increase,colour='CPI%'),size=2)+geom_line(aes(y=AWI_Increase,colour='AWI%'),size=2)+scale_color_manual("",breaks=c('CPI%','AWI%'),values=c('red','blue'))+scale_x_continuous(breaks=c(2012,2013,2014))


scatterplot1 <- S_PL%>%group_by(MntProducts,Year_Birth)%>%select(c(2,5,6,7,23,24))%>%ggplot()+geom_point(
    aes(x=Year_Birth,y=Dollars,colour=MntProducts))+ggtitle('Amount Spent on Different Products')+scale_x_continuous(limits=c(1945,1996))


scatterplot2 <- S_PL%>%group_by(MntProducts,Income)%>%select(c(2,5,6,7,23,24))%>%ggplot()+geom_point(
    aes(x=Income,y=Dollars,colour=MntProducts))+ggtitle('Amount Spent on Different Products')+scale_x_continuous(limits=c(2000,120000)) 


lineplot2 <- S_PL2%>%group_by(NumPhurchases,Year_Birth)%>%select(c(2,5,6,7,24,25))%>%summarise(meanfreq=mean(Number))%>%ggplot()+geom_line(
    aes(x=Year_Birth,y=meanfreq,colour=NumPhurchases))+ggtitle('Purchase Frequency on Different Products')+scale_x_continuous(limits=c(1945,1996)) 


densityplot1 <- S_PL2%>%group_by(NumPhurchases,Income)%>%select(c(2,5,6,7,24,25))%>%summarise(meanfreq=mean(Number))%>%ggplot(aes(x=Income))+geom_density(aes(y=meanfreq,colour=NumPhurchases),stat='identity')+ggtitle('Purchase Frequency on Different Products')+scale_x_continuous(limits=c(2000,120000))

rsconnect::setAccountInfo(name='cx626shiny', token='1E07FB67DCD1ED297B707B01722DF4DE', secret='a8DkfoRDV98VqCpBMkT+eRKdmHMtnJjIAIJ3F8a4')
